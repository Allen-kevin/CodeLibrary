...
//	printf("total = %d, epoch = %d, fd = %d, avg = %d, usec_offset = %d, vir_delay = %d\n", total, epoch, fd, average, usec_offset, virtual_delay);
...
int canSendPkt(const int fd, const int epoch)
{
	...
	if (packets_time[fd].efd == 0 || packets_time[fd].connectionClosed == 1) {
		return FALSE;
	}
	struct timeval now;
	gettimeofday(&now, NULL);
...
int controlSendingRate(const int fd, const unsigned int prop)
{
	if (prop < 0 || prop >= 100)
		return SENDING;
