...
void *throughput_sec(void *context)
{
	while (1) {
		sleep(1);
		if (stats.send_counter_sec > 0 && (stats.request_counter_sec == 0 || stats.receive_counter_sec == 0))
			printf("###3\n");
